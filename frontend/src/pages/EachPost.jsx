  ...
  const handleReactionSelect = async (reactionType) => {
    ...
    try {
      const response = await axios.post(
        ...
        { userId: user._id, type: reactionType },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      if (response.data.message === "Reaction removed") {
        setReactions((prevReactions) =>
          prevReactions.filter((r) => r.userId?._id !== user._id)
        );
        ...
        setReactions((prevReactions) =>
          prevReactions.map((r) =>
            r.userId?._id === user._id ? { ...r, type: reactionType } : r
          )
        );
              ...
              <div key={i} className="flex space-x-2">
                ...
                <div className="flex-1">
                  <div className="bg-gray-100 rounded-lg p-2">
                    <p className="font-semibold text-sm">Sample User</p>
          ...
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md">
            ...
            <div className="p-2 flex border-b overflow-x-auto">
              ...
              {getDistinctReactionIcons().map((icon, index) => {
                const reactionType = reactionTypes.find(
                  (r) => r.icon === icon
                );
                return (
                  <button
